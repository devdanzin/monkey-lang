...
- **Monkey language** — Interpreter + bytecode compiler + stack VM + **tracing JIT compiler**. 843+ tests, 30 benchmarks. ~8x aggregate speedup (dot-product 29.7x!). 12 optimizer passes. Day 10: 30+ language features (match, destructuring, for-in, slicing, ternary, ??, ?., |>, arrows, spread, rest, const, etc.), monkey-to-JS transpiler, interactive playground, 10 example programs, 25 builtins. Blog posts: "Range Check Elimination", "Growing a Language", "When Optimizers Attack".
