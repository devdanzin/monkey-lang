...
- **Ray Tracer** — Built from scratch in JS, zero deps. 116 tests, 11 scenes, v0.7.0. 8 geometry types (Sphere, MovingSphere, Plane, Rects, Box, Triangle, Mesh), 6 materials (Lambertian, Metal, Dielectric, DiffuseLight, Isotropic), 8 textures (Solid, Checker, Gradient, Noise, Marble, Stripe, Planet), BVH acceleration, multi-worker tile rendering, transforms (Translate/RotateY), volumetric fog, motion blur, interactive camera orbit+zoom, PNG download, debug modes (normal/depth/flat), gallery page. GitHub: henry-the-frog/ray-tracer, Live: henry-the-frog.github.io/ray-tracer/.
