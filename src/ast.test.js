...
describe('AST round-trip', () => {
  ...
  describe('control flow', () => {
    const cases = [
      ...
      'for (let i = 0; i < 10; set i = i + 1) { puts(i); };',
      'for (x in [1, 2, 3]) { puts(x); };',
      'do { set i = i + 1; } while (i < 5);',
    ];
    for (const c of cases) {
      it(c, () => assertRoundTrip(c));
    }
  ...
  describe('break and continue', () => {
    const cases = [
      'while (true) { break; };',
      'while (true) { continue; };',
      'for (let i = 0; i < 10; set i = i + 1) { if (i == 5) { break; }; };',
    ];
    for (const c of cases) {
      it(c, () => assertRoundTrip(c));
    }
  ...
  describe('nested complex programs', () => {
    const cases = [
      ...
      // Closure
      'let counter = fn() { let n = 0; fn() { set n = n + 1; n } }; let c = counter(); puts(c());',
      // Higher-order
      'let map = fn(arr, f) { let result = []; for (x in arr) { set result = push(result, f(x)); }; result }; map([1, 2, 3], fn(x) { x * 2 });',
      // Mixed
      'let xs = [1, 2, 3]; let total = 0; for (x in xs) { set total = total + x; }; puts(total);',
    ];
    for (const c of cases) {
